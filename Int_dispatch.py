# -*- coding: utf-8 -*-
"""
Created on Thu Feb 17 11:43:43 2022
...
#DATA
time = indata.time 
time_numbers =indata.time_numbers
gen= indata.gen
wind= indata.wind
CHP= indata.CHP
...
ax1.stackplot(time_numbers, p_DA_all.values(),
...
ax1.text(8, 140, E_DA['CHP1'])
ax1.text(8, 340, E_DA['CHP2'])
ax1.text(8, 540, E_DA['G2'])
ax1.text(8, 740, E_DA['W1'])
...
ax.stackplot(time_numbers, q_chp.values(),
             labels = q_chp.keys(), alpha=0.8)
...
for s in integ.CHP:
    profit_t=[]
    for t in integ.T:
        profit_t.append(profit_it[s,t])
    profit_chp[s]=profit_t
...
ax4.legend(h1+h2, l1+l2, fancybox=True, shadow=True)
...
# ax6.bar(indata.time_numbers,q_chp['CHP2'], fill=False)
# ax7 = ax6.twinx()
# #color = 'k'
# ax7.plot(list(lambda_DA_H.values()), label='Actual Price')
# plt.show()
# =============================================================================
#ax.stackplot(time, q_chp.values(),
#             labels = q_chp.keys(), alpha=0.8)
