# -*- coding: utf-8 -*-
"""
Created on Thu Feb 17 11:47:27 2022
...
def seq_dispatch_func(indata, L_DA_E_F):
    ...
    heat.u_DA=pe.Var(heat.CHP, heat.T, domain=pe.NonNegativeReals)
    ...
    elec.u_DA=pe.Var(elec.G, elec.T, domain=pe.NonNegativeReals)
...
# =============================================================================
    ...
    plt.savefig('Heat_seq.pdf', dpi=100, bbox_inches='tight')
    plt.show    
    ...
    plt.savefig('Power_seq.pdf', dpi=100, bbox_inches='tight')
    plt.show  
    ...
    plt.savefig('Exp_Power_Estim_Price_seq.pdf', dpi=100, bbox_inches='tight')
    plt.show()
    ...
    ax7.set_ylim(-10, 300)
    plt.savefig('CHP_dispatch_seq_big.pdf', dpi=100, bbox_inches='tight')
    plt.show
    ...
    for s in heat.CHP:
        u_t=[]
        for t in heat.T:
            u_t.append(pe.value(heat.u_DA[s,t]))
        u_DA_all[s]=u_t
    for g in elec.G:
        u_t=[]
        for t in elec.T:
            u_t.append(pe.value(elec.u_DA[g,t]))
        u_DA_all[g]=u_t
# =============================================================================
...
# =============================================================================
    ...
    return Wind_curt_total, total_cost1, lambda_DA_E_seq, lambda_DA_H   
